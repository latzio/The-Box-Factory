...
class Moveable {
   ...
   virtual Type getType() { return TYPE_MOVEABLE; }
...
class Spark : public Particle {
  public:
    ...
    virtual Type getType() { return TYPE_SPARK; }
  protected:
       virtual void addGravity();
};
...
class Obstacle : public Moveable {
  public:
    ...
    virtual Moveable* IsHit(const Point3D& p, double r);
    virtual Type getType() { return TYPE_OBSTACLE; }
  protected:
    Point3D m_centre;
    double m_nRadius;
};
...
class NPC : public Moveable, public AISubscriber {
  ...
  public:
   ...
   // NPCs have health
   virtual void setHealth(int nHealth) { m_nHealth = nHealth; }
   virtual bool damage(int nHealth); // return true if health < 0 after damage
    ...
    virtual void setThrottle(double throttle) { m_nThrottle = throttle; }
...
class PistolNPC : public NPC {
    ...
    virtual Type getType() { return TYPE_NPC_PISTOL; }
...
class PC : public NPC {
    ...
    virtual void setHumanControlled(bool b) { m_bHumanControlled = b; }
    virtual bool isHumanControlled()  { return m_bHumanControlled; }
